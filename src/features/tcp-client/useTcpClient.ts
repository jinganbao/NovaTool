...
export function useTcpClient() {
  ...
  function saveConnection() {
    ...
    const existing = selectedConnectionId.value
      ...
      : null;
    if (existing) {
      Object.assign(existing, { name: cleanName, host: host.value.trim(), port: port.value.trim(), mode: mode.value });
  ...
  async function sendPayload() {
    ...
    appendLog("send", `${endpoint.host}:${endpoint.port} ${mode.value.toUpperCase()} ${persistentId ? "[长连接] " : ""}${payload.value}`);
    try {
      const result = await invoke<TcpSendResult>(
        persistentId ? "tcp_conn_send" : "tcp_send",
        persistentId
          ? { connId: persistentId, payload: payload.value, mode: mode.value, timeoutMs: endpoint.timeoutMs }
          : { ...endpoint, payload: payload.value, mode: mode.value },
      );
      if (!persistentId) {
  ...
  return {
    connId, connecting, disconnecting, host, port, connectionName, mode, timeoutMs,
    selectedConnectionId, connectionOptions, payload, sending, selectedPayloadId,
    payloadName, payloadOptions, showTemplate, templateFields, templateSuffix,
    templateResult, receiveMode, receiveLog, receiveRef, copyText, connect, disconnect,
    applyConnection, saveConnection, deleteConnection, applySavedPayload, savePayload,
    deletePayload, addTemplateField, removeTemplateField, fillTemplateToPayload, sendPayload,
  };
}
